...
#include "ClosedChainMechSolver.h"
...
double ClosedChainMechSolver::Solve()
{
	...
	const auto MechanismSolveTolerance = GConfig.Get<double>("DesignerConfig", "MechanismSolveTolerance");
...
void ClosedChainMechSolver::Drive(ObjectPtr<Joint> Root, double DeltaTime)
{
	Root->GlobalTransform.AddRotationLocal(MMath::QuaternionFromEulerXYZ(
		{ 0, 0, DrivenDir * DeltaTime}));

	if(!bLoop)
	{
		CurrentDrivenAngle += DrivenDir * DeltaTime;
		if (CurrentDrivenAngle < 0. || CurrentDrivenAngle > MaxiumDrivenAngle)
			DrivenDir *= -1.;
	}
}
void ClosedChainMechSolver::SetLimitDrivenAngle(double MaxiumAngle)
{
	bLoop = false;
	MaxiumDrivenAngle = MaxiumAngle;
}

double ClosedChainMechSolver::CalcSingularity(const VectorXd& Parameter) const
{
	MatrixXd jacFK;
	ClosedChainMechFunctor LossFunc(ParameterNum());
	LossFunc.Joints = Joints;
